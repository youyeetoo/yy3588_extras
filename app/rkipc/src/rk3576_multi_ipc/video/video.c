...
int rkipc_bind_init() {
	...
	for (int i = 0; i < g_sensor_num; i++) {
		...
		LOG_INFO("bind success %#x: vi [%d, %d] -> avs [%d, %d]\n", ret, vi_chn[i].s32DevId,
		         vi_chn[i].s32ChnId, avs_in_chn[i].s32DevId, avs_in_chn[i].s32ChnId);
	}
...
int rkipc_osd_bmp_create(int id, osd_data_s *osd_data) {
	...
	stVpssChn.s32ChnId = VPSS_GRP_ID;
	...
	ret = RK_MPI_RGN_AttachToChn(RgnHandle, &stVpssChn, &stRgnChnAttr);
	if (RK_SUCCESS != ret) {
...
int rkipc_osd_bmp_destroy(int id) {
	...
	stVpssChn.s32DevId = 0;
	stVpssChn.s32ChnId = VPSS_GRP_ID;
	ret = RK_MPI_RGN_DetachFromChn(RgnHandle, &stVpssChn);
	if (RK_SUCCESS != ret)
