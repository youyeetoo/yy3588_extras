...
void GetFat(FAT_ENTRY * entry, void *fat, uint32_t cluster, DOS_FS * fs)
{
    ...
    if (cluster > fs->data_clusters + 1) {
        Die("Internal error: cluster out of range in GetFat() (%lu > %lu).",
...
int ReadFat(DOS_FS * fs)
{
    ...
    if (fs->fat_bits != 12)
         ...
         * casing the last entry in GetFat() */
        alloc_size = (total_num_clusters * 12 + 23) / 24 * 3;

    first = malloc(alloc_size);
    if (FsRead(fs, fs->fat_start, eff_size, first))
        goto err;
    if (fs->nfats > 1) {
        second = malloc(alloc_size);
        if (FsRead(fs, fs->fat_start + fs->fat_size, eff_size, second))
            goto err;
    }
    if (second && memcmp(first, second, eff_size) != 0) {
        FAT_ENTRY first_media, second_media;
        GetFat(&first_media, first, 0, fs);
        GetFat(&second_media, second, 0, fs);
        first_ok = (first_media.value & FAT_EXTD(fs)) == FAT_EXTD(fs);
        second_ok = (second_media.value & FAT_EXTD(fs)) == FAT_EXTD(fs);
        if (first_ok && !second_ok) {
            printf("FATs differ - using first FAT.\n");
            FsWrite(fs, fs->fat_start + fs->fat_size, eff_size, first);
        }
        if (!first_ok && second_ok) {
            printf("FATs differ - using second FAT.\n");
            FsWrite(fs, fs->fat_start, eff_size, second);
    ...
    fs->cluster_owner = malloc(total_num_clusters * sizeof(DOS_FILE *));
...
void SetFat(DOS_FS * fs, uint32_t cluster, int32_t new)
{
    ...
    if (cluster > fs->data_clusters + 1) {
        Die("Internal error: cluster out of range in SetFat() (%lu > %lu).",
              (unsigned long)cluster, (unsigned long)(fs->data_clusters + 1));
    }
    ...
    if (new == -1)
        ...
        Die("Internal error: new cluster out of range in SetFat() (%lu > %lu).",
    ...
    case 12:
        ...
        if (cluster & 1) {
            FAT_ENTRY prevEntry;
            GetFat(&prevEntry, fs->fat, cluster - 1, fs);
            data[0] = ((new & 0xf) << 4) | (prevEntry.value >> 8);
            data[1] = new >> 4;
        } else {
            FAT_ENTRY subseqEntry;
            if (cluster != fs->data_clusters + 1)
                GetFat(&subseqEntry, fs->fat, cluster + 1, fs);
            else
                subseqEntry.value = 0;
            data[0] = new & 0xff;
    ...
    case 32: {
        FAT_ENTRY curEntry;
        GetFat(&curEntry, fs->fat, cluster, fs);
    ...
    FsWrite(fs, offs, size, data);
    if (fs->nfats > 1) {
        FsWrite(fs, offs + fs->fat_size, size, data);
    }
}
...
uint32_t NextCluster(DOS_FS * fs, uint32_t cluster)
{
    ...
    if (FAT_IS_BAD(fs, value))
        Die("Internal error: NextCluster on bad cluster");
...
void FixBad(DOS_FS * fs)
{
    ...
    for (i = 2; i < fs->data_clusters + 2; i++) {
        ...
        if (!GetOwner(fs, i) && !FAT_IS_BAD(fs, curEntry.value))
            if (!FsCheck(fs, ClusterStart(fs, i), fs->cluster_size)) {
...
void ReclaimFree(DOS_FS * fs)
{
    ...
    for (i = 2; i < fs->data_clusters + 2; i++) {
        ...
        if (!GetOwner(fs, i) && curEntry.value &&
                     ...
                     uint32_t start_cluster)
{
    ...
    for (i = start_cluster; i < fs->data_clusters + 2; i++) {
        ...
        if (curEntry.value && !FAT_IS_BAD(fs, curEntry.value) &&
            !GetOwner(fs, i) && !num_refs[i]) {
            prev = 0;
            /* Walk the chain, claiming ownership as we go */
            for (walk = i; walk != -1; walk = NextCluster(fs, walk)) {
                if (!GetOwner(fs, walk)) {
                    SetOwner(fs, walk, owner);
                } else {
...
void ReclaimFile(DOS_FS * fs)
{
    ...
    total_num_clusters = fs->data_clusters + 2;
    num_refs = malloc(total_num_clusters * sizeof(uint32_t));
    ...
    for (i = 2; i < total_num_clusters; i++)
        ...
        if (GetOwner(fs, i) == &orphan && !num_refs[i]) {
#if 0
            DIR_ENT de;
            off_t offset;
            files++;
            offset = AllocRootEntry(fs, &de, "FSCK%04dREC", 1);
...
uint32_t UpdateFree(DOS_FS * fs)
{
    ...
    if (do_set) {
        ...
        FsWrite(fs, fs->fsinfo_start + offsetof(struct info_sector, free_clusters),
