...
/* get size in bytes */
int
BlkDevGetSize(int fd, unsigned long long *bytes)
{
    ...
    *bytes = BlkDevFindSize(fd);
    return 0;
}

/* get 512-byte sector count */
int
BlkDevGetSectors(int fd, unsigned long long *sectors)
{
    unsigned long long bytes;

    if (BlkDevGetSize(fd, &bytes) == 0) {
        *sectors = (bytes >> 9);
        return 0;
    }
 ...
 */
int BlkDevGetSectorSize(int fd, int *sector_size)
{
#ifdef BLKSSZGET
 ...
 * Example:
 *
 * rc = BlkDevGetPhysectorSize(fd, &physec);
 * if (rc || physec == 0) {
 *  rc = BlkDevGetSectorSize(fd, &physec);
 *  if (rc)
 *      physec = DEFAULT_SECTOR_SIZE;
 * }
 */
int BlkDevGetPhysectorSize(int fd, int *sector_size)
{
#ifdef BLKPBSZGET
 ...
 */
int BlkDevGetGeometry(int fd, unsigned int *h, unsigned int *s)
{
#ifdef HDIO_GETGEO
    {
 ...
 */
int BlkDevGetStart(int fd, unsigned int *s)
{
#ifdef HDIO_GETGEO
    struct hd_geometry geometry;
