...
void FileAdd(FDSC *fp_root, char *path, FD_TYPE type)
{
    ...
    while (1) {
        if ((here = strchr(path, '/')))
            * here = 0;
        if (!FileCvt((unsigned char *)path, (unsigned char *)name))
            exit(2);
    ...
    while (this) {
        ...
        if (this->first)
            ...
            printf("Warning: did not %s file %s\n", this->type == fdt_drop ?
                   "drop" : "undelete", FileName((unsigned char *)this->name));
        free(this);
        this = next;
    }
}
